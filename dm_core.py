...
def create_initial_game():
    ...
    def create_player(name):
        deck = random.sample(sample_deck, 40)  # ランダムなデッキ（実際には固定デッキでも可）
...
def summon_creature_to_battle_zone(player, creature, creature_card, from_effect=False):
    ...
    player.battle_entry_log.append({
        'id': creature_card.id,
        'used': False
    })
...
def process_battle_zone_effects(player, game):
    for entry in player.battle_entry_log:
        # すでに能力を使ったものはスキップする例
        if entry["used"]:
            continue
        # カードオブジェクト本体を探す
        target = next((c for c in player.battle_zone if c.id == entry["id"]), None)
        if target and hasattr(target, "on_end_of_turn"):
            target.on_end_of_turn(player, game)
            # 処理したらフラグを立てて重複を防ぐ
            entry["used"] = True
...
class RuleBasedAI:
    ...
    def play_cards(self, game):
        player = game.players[self.player_id]
        print("[DEBUG][AI] カード召喚直後 battle_zone:", [c.name for c in player.battle_zone])
        ...
        for card in boost_only:
            if card.cost <= player.available_mana and card in player.hand:
                play_card_for_ai(game, player, player.hand.index(card))
                print("[DEBUG][AI] 通常カードプレイ後 battle_zone:", [c.name for c in player.battle_zone])
                break

        print("[DEBUG][AI] play_cards終了時 battle_zone:", [c.name for c in player.battle_zone])
        print("[DEBUG][AI] play_cards終了時 hand:", [c.name for c in player.hand])
...
def take_turn_ai(game):
    ...
    if player_id == AI_PLAYER_ID:
        ...
        ai.play_cards(game)
        ai.attack(game)
        print("[DEBUG][AIターン終了直前] AIのbattle_zone:", [c.name for c in game.players[1].battle_zone])
        print("[DEBUG][AIターン終了直前] AIのhand:", [c.name for c in game.players[1].hand])
        print("[DEBUG][AIターン終了直前] AIのmana_zone:", [c.name for c in game.players[1].mana_zone])
        end_turn(game)
        return